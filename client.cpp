...
void Client::DrawHUD() {
...
#ifdef _DEBUG
	std::string text = tfm::format(XOR("supremacy %i ms + %s rate"), ms, rate);
	...
	// text.
	render::menu.string(m_width - 15, 10, { 255, 255, 255, 255 }, text, render::ALIGN_RIGHT);
}
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::green, XOR("[supremacy] "));
