...
vec3_t math::extend_vector( vec3_t pos, float length, float angle ) {
	auto rad{ angle * math::pi / 180.f };
	return vec3_t( pos.x + ( cos( rad ) * length ), pos.y + ( sin( rad ) * length ), pos.z );
}

vec3_t math::extrapolate_pos( vec3_t pos, vec3_t extension, int amount ) {
	return pos + ( extension * ( g_csgo.m_globals->m_interval * amount ) );
}