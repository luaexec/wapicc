...
void Dropdown::draw() {
	...
	// draw border.
	render::rect(p.x + DROPDOWN_X_OFFSET, p.y + m_offset, m_w - DROPDOWN_X_OFFSET, DROPDOWN_BOX_HEIGHT, g_gui.light_border);

	// draw outer border.
	render::rect(p.x + DROPDOWN_X_OFFSET - 1, p.y + m_offset - 1, m_w - DROPDOWN_X_OFFSET + 2, DROPDOWN_BOX_HEIGHT + 2, g_gui.dark_border);
	...
	if (!m_items.empty()) {
		...
		if (m_open) {
			...
			// draw items outline.
			render::rect(p.x + DROPDOWN_X_OFFSET, p.y + m_offset + DROPDOWN_BOX_HEIGHT + DROPDOWN_SEPARATOR, m_w - DROPDOWN_X_OFFSET, m_anim_height + 1, g_gui.light_border);

			// draw items inside.
			render::rect_filled(p.x + DROPDOWN_X_OFFSET + 1, p.y + m_offset + DROPDOWN_BOX_HEIGHT + DROPDOWN_SEPARATOR + 1, m_w - DROPDOWN_X_OFFSET - 2, m_anim_height - 1,  g_gui.background );
