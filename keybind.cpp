...
void Keybind::draw() {
	...
	if (!m_set && !(m_key >= 0 && m_key <= 0xFE))
		render::menu.string(p.x - render::menu.size(XOR("[-]")).m_width + m_w + 1, p.y + 15 + 4 - 21, color, XOR("[-]"));
    ...
    // we have a key assigned.
        ...
        render::menu.string(p.x - render::menu.size(tfm::format("%s", keynames[m_key])).m_width + m_w + 1, p.y + 15 + 4 - 21, { color.r(),  color.g(),  color.b(), m_parent->m_alpha }, tfm::format("%s", keynames[m_key]));
}
...
void Keybind::click() {
	...
	Rect bind = { p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET + 120, p.y + 15 + 4 - 21, m_w + 20, KEYBIND_BOX_HEIGHT };
