...
void Keybind::draw( ) {
	...
	// draw label.
	render::menu.string( p.x + KEYBIND_X_OFFSET, p.y - 2, { 152, 152, 152, m_parent->m_alpha }, m_label );

	// box outline.
	render::rect( p.x + KEYBIND_X_OFFSET, p.y + 15, m_w - KEYBIND_X_OFFSET, KEYBIND_BOX_HEIGHT, { 209,209,209, m_parent->m_alpha } );

	// inner blob
	// todo; gradient?
	render::rect_filled( p.x + KEYBIND_X_OFFSET + 1, p.y + 15 + 1, m_w - KEYBIND_X_OFFSET - 2, KEYBIND_BOX_HEIGHT - 2, { 209,209,209, m_parent->m_alpha } );
	...
	// we have a key assigned.
		...
		render::menu.string( p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET, p.y + 15 + 4, { 145, 145, 145, m_parent->m_alpha }, keynames[ m_key ] );
}
