...
void MultiDropdown::draw( ) {
	...
	// draw border.
	render::rect( p.x + DROPDOWN_X_OFFSET, p.y + m_offset, m_w - DROPDOWN_X_OFFSET, DROPDOWN_BOX_HEIGHT, { 208,208,208, m_parent->m_alpha } );

	// draw inside.
	render::rect_filled( p.x + DROPDOWN_X_OFFSET + 1, p.y + m_offset + 1, m_w - DROPDOWN_X_OFFSET - 2, DROPDOWN_BOX_HEIGHT - 2, { 214,214,214, m_parent->m_alpha } );
	...
	if( !m_items.empty( ) ) {
		...
		if( m_open ) {
			...
			// draw items outline.
			render::rect( p.x + DROPDOWN_X_OFFSET, p.y + m_offset + DROPDOWN_BOX_HEIGHT + DROPDOWN_SEPARATOR, m_w - DROPDOWN_X_OFFSET, m_anim_height + 1, { 208,208,208, m_parent->m_alpha } );

			// draw items inside.
			render::rect_filled( p.x + DROPDOWN_X_OFFSET + 1, p.y + m_offset + DROPDOWN_BOX_HEIGHT + DROPDOWN_SEPARATOR + 1, m_w - DROPDOWN_X_OFFSET - 2, m_anim_height - 1, { 200,200,200, m_parent->m_alpha } );
