...
//	render::rect ( p.x + BUTTON_X_OFFSET, p.y, m_w - BUTTON_X_OFFSET, BUTTON_BOX_HEIGHT, { 42, 42, 42, m_parent->m_alpha } );
	...
	// bar.
	render::rect_filled( p.x + SLIDER_X_OFFSET + 2, p.y + m_offset + 2, m_fill, SLIDER_HEIGHT - 4, color );
	...
	// draw value.
	render::menu.wstring( p.x + m_fill - ( size.m_width / 2 ), p.y + m_offset - 4, { 255, 255, 255, m_parent->m_alpha }, ss.str( ) );
