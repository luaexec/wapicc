...
void Slider::draw( ) {
	...
	// outline.
	render::rect( p.x + SLIDER_X_OFFSET, p.y + m_offset, m_w - SLIDER_X_OFFSET, SLIDER_HEIGHT, { 217,217,217, m_parent->m_alpha } );

	// background.
	render::gradient( p.x + SLIDER_X_OFFSET + 1, p.y + m_offset + 1, m_w - SLIDER_X_OFFSET - 2, SLIDER_HEIGHT - 2, { 217,217,217, m_parent->m_alpha }, { 217,217,217, m_parent->m_alpha } );

	// bar.
	render::rect_filled( p.x + SLIDER_X_OFFSET + 1, p.y + m_offset + 1, m_fill - 2, SLIDER_HEIGHT - 2, color );
//	render::rect_filled_fade( p.x + SLIDER_X_OFFSET + 1, p.y + m_offset + 1, m_fill - 2, SLIDER_HEIGHT - 2, { 217,217,217, m_parent->m_alpha }, 0, 150 );
	...
	// draw value.
	render::menu_shade .wstring( p.x + SLIDER_X_OFFSET + m_fill - ( size.m_width / 2 ), p.y + m_offset + 1, { 200, 200, 200, m_parent->m_alpha }, ss.str( ) );
